...
namespace CreepyCrawly.LanguageDefinition
{
    public class ExecutionPlanListener: CreepyCrawly.LanguageEngine.CrawlLangListener
    {
        ...
        private List<IPartialCommand> _CurrentPartialCommands;
        ...
        public override void EnterPartial_extract_text_command([NotNull] CrawlLangParser.Partial_extract_text_commandContext context)
        {
            string selector = context.selector().GetText().Trim('\'');
            string regex = context.regex()?.GetText().Trim('r').Trim('\'');
            PartialExtractTextCommand extractTextCommand = new PartialExtractTextCommand(selector, _ExecutionEngine.ExtractText, regex);
            _CurrentPartialCommands.Add(extractTextCommand);
        }

        public override void EnterPartial_extract_title_command([NotNull] CrawlLangParser.Partial_extract_title_commandContext context)
        {
            PartialExtractTitleCommand extractTitleCommand = new PartialExtractTitleCommand(_ExecutionEngine.ExtractTitle);
            _CurrentPartialCommands.Add(extractTitleCommand);
        }

        public override void EnterPartial_extract_image_command([NotNull] CrawlLangParser.Partial_extract_image_commandContext context)
        {
            string selector = context.selector().GetText().Trim('\'');
            PartialExtractImageCommand extractImageCommand = new PartialExtractImageCommand(selector, _ExecutionEngine.ExtractImage);
            _CurrentPartialCommands.Add(extractImageCommand);
        }

        public override void EnterPartial_extract_script_command([NotNull] CrawlLangParser.Partial_extract_script_commandContext context)
        {
            string selector = context.selector().GetText().Trim('\'');
            PartialExtractScriptCommand extractScriptCommand = new PartialExtractScriptCommand(selector, _ExecutionEngine.ExtractScript);
            _CurrentPartialCommands.Add(extractScriptCommand);
        }

        public override void EnterPartial_extract_href_command([NotNull] CrawlLangParser.Partial_extract_href_commandContext context)
        {
            string selector = context.selector().GetText().Trim('\'');
            PartialExtractHrefCommand extractHrefCommand = new PartialExtractHrefCommand(selector, _ExecutionEngine.ExtractHref);
            _CurrentPartialCommands.Add(extractHrefCommand);
        }

        public override void EnterComposite_extract_command([NotNull] CrawlLangParser.Composite_extract_commandContext context)
        {
            _CurrentPartialCommands = new List<IPartialCommand>();
        }

        public override void ExitComposite_extract_command([NotNull] CrawlLangParser.Composite_extract_commandContext context)
        {
            CompositeExtractCommand compositeExtractCommand = new CompositeExtractCommand(_CurrentPartialCommands);
            _CurrentPartialCommands = null;
            _CurrentBlock.Add(compositeExtractCommand);
        }
    }
}
