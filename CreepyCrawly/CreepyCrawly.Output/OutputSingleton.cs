...
namespace CreepyCrawly.Output
{
    public class OutputSingleton
    {
        ...
        public static void WriteToImageOutputtersWithFileName(object output, string fileName)
        {
            _Outputters.Where(_ => _ is ImageFileOutputter)
                       .ToList()
                       .ForEach(_ => ((ImageFileOutputter)_).WriteOutputWithName(output.ToString(), fileName));
        }
